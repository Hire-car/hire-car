      ...
      <div className="p-3 border-b border-slate-100 flex-shrink-0 bg-slate-50/50">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-white border border-slate-200 rounded-xl pl-9 pr-4 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#FF5F00]/50 transition-all"
          />
        </div>
      </div>
              ...
              return (
                <Link 
                  ...
                  className={`
                    ...
                    ${isActive ? 'bg-[#FF5F00]/5 hover:bg-[#FF5F00]/10' : 'hover:bg-slate-50'}
                  `}
                >
                  <div className={`h-12 w-12 rounded-full border border-slate-100 flex items-center justify-center font-bold text-sm shrink-0 ${isActive ? 'bg-[#FF5F00]/10 text-[#FF5F00]' : 'bg-slate-100 text-slate-600'}`}>
                    {chat.initials}
                  </div>
                  
                  <div className="flex-1 min-w-0 flex flex-col justify-center h-12">
                    <div className="flex justify-between items-center mb-0.5">
                      <h3 className={`text-sm font-semibold truncate pr-2 ${isActive ? 'text-[#FF5F00]' : 'text-slate-900'}`}>
                        {chat.name}
                      </h3>
                      <span className={`text-[11px] whitespace-nowrap flex-shrink-0 ${isActive ? 'text-[#FF5F00]' : 'text-slate-400'}`}>
                        {timeString}
                      </span>
                    </div>
