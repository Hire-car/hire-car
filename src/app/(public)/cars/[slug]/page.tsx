      ...
      <main className="mx-auto max-w-7xl px-4 py-8 pb-28 lg:pb-8 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_380px] lg:gap-12 mt-4">
          ...
          <div className="space-y-6 min-w-0">
            ...
            {/* Specs Grid (Moved out of title card) */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                <div key={label} className="flex flex-col items-center justify-center rounded-xl bg-card border border-border p-5 text-center shadow-sm">
                  <Icon className="h-6 w-6 text-primary mb-3" strokeWidth={1.5} />
                  <p className="text-xs font-semibold text-muted-foreground mb-1">{label}</p>
                  <p className="text-sm font-bold text-foreground truncate w-full">{value}</p>
                </div>
              ))}
            </div>
          ...
          <aside className="space-y-6 lg:sticky lg:top-24 h-fit scroll-mt-[120px]" id="enquiry-section">
            {/* Title & Details (Moved to right column) */}
